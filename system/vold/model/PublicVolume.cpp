...
status_t PublicVolume::readMetadata() {
    ...
    mFsUuid = StringPrintf("%s%d",mFsUuid.c_str(),user_id);
    if(user_id == 10){
        mainPath = mFsUuid;
        PLOG(WARNING) << getId() << " record user 10 mainPath = " << mainPath;
    }

    PLOG(WARNING) << getId() << " update uuid to storagemanger = " << mFsUuid;
...
status_t PublicVolume::doMount() {
    ...
    property_get("ro.target.product", product, "");
    readMetadata();

    if((!strcmp(product, "car") || !strcmp(product, "vehicle")) && user_id != 10 ){
        PLOG(ERROR) << getId() << " ignore check for other user " << user_id;
    }else{
    ...
    if(!strcmp(product, "car") || !strcmp(product, "vehicle"))
    	stableName = StringPrintf("%s%d",getId().c_str(),user_id);
    if (!mFsUuid.empty()) {
        stableName = mFsUuid;
    }
    ...
    if((!strcmp(product, "car") || !strcmp(product, "vehicle")) && user_id > 10 )
    {
	mRawPath = StringPrintf("/mnt/media_rw/%s", mainPath.c_str());
	LOG(ERROR) << " ---force set mRawPath to  " << mRawPath << " for user" << user_id;
    }
    ...
    LOG(ERROR) << " ---stableName= " << stableName << " mRawPath= " << mRawPath;
    ...
    if((!strcmp(product, "car") || !strcmp(product, "vehicle")) && user_id != 10 ){
    	PLOG(ERROR) << getId() << " ignore really mount for other user " << user_id;
    }else{
   	 if (mFsType == "vfat") {
    ...
    LOG(INFO) << "Mounting public fuse volume for uid= "<< user_id << " getInternalPath= "<< getInternalPath() << " stableName= "<<stableName;
