...
AIC_WIFI_KO_FILES := $(shell find $(TOPDIR)$(PRODUCT_KERNEL_PATH)/drivers/net/wireless/aic8800 -name "*.ko" -type f)
AIC_BTUSB_KO_FILES := $(shell find $(TOPDIR)$(PRODUCT_KERNEL_PATH)/drivers/bluetooth -name "*.ko" -type f)
BOARD_VENDOR_KERNEL_MODULES += \
	$(foreach file, $(WIFI_KO_FILES), $(file)) \
	$(foreach file, $(AIC_WIFI_KO_FILES), $(file)) \
	$(foreach file, $(AIC_BTUSB_KO_FILES), $(file))
