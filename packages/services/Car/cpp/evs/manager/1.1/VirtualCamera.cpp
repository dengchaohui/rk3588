...
void VirtualCamera::shutdown() {
    LOG(DEBUG) << "Virtual camera " << mHalCamera.begin()->first << " shutdown ";
    {
    ...
    mHalCamera.clear();
    LOG(DEBUG) << "Virtual camera shutdown done";
}
...
Return<EvsResult> VirtualCamera::startVideoStream(const ::android::sp<IEvsCameraStream_1_0>& stream)  {
    ...
    while (iter != mHalCamera.end()) {
        ...
        if ((!result.isOk()) || (result != EvsResult::OK)) {
            LOG(ERROR) << "Failed to start a client video stream on " << iter->first;
    ...
    if (mStream_1_1 != nullptr && pHwCamera != nullptr) {
        mCaptureThread = std::thread([this]() {
            ...
            while (true) {
                ...
                if (mStreamState != RUNNING || !mStream_1_1) {
                    // A video stream is stopped while a capture thread is waiting
                    // for a new frame or we have lost a client.
                    LOG(DEBUG) << "Requested to stop capturing frames or lost a client, state "
                               << mStreamState << " stream " << (mStream ? mStream_1_1.get() : nullptr);
...
Return<void> VirtualCamera::stopVideoStream() {
    {
        ...
        if ((!mStream && !mStream_1_1) || mStreamState != RUNNING) {
