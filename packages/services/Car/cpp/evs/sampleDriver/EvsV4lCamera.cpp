...
const std::array<int32_t, 2> kDefaultResolution = {1920, 1080};
...
void EvsV4lCamera::forwardFrame(imageBuffer* pV4lBuff, void* pData) {
    ...
    if (mDumpFrame) {
        ...
        if (fd == -1) {
            ...
            LOG(INFO) << pV4lBuff->m.planes[0].length;
            ...
            len += write(fd.get(), pData, pV4lBuff->m.planes[0].length);
...
sp<EvsV4lCamera> EvsV4lCamera::Create(const char *deviceName,
    ...
    if (!success) {
        ...
        if (camInfo != nullptr)
            LOG(INFO) << "Open a video in xml config with "
                      << "width: " << camInfo->streamConfigurations[0][1]
                      << ", height: " << camInfo->streamConfigurations[0][2];
            success =
                evsCamera->mVideo.open(deviceName, camInfo->streamConfigurations[0][1], camInfo->streamConfigurations[0][2]);
            if (!success) {
                LOG(ERROR) << "Failed to open a video stream";
                return nullptr;
            }
        else {
            LOG(INFO) << "Open a video with default parameters with "
                      << "width: " << kDefaultResolution[0]
                      << ", height: " << kDefaultResolution[1];
            success =
