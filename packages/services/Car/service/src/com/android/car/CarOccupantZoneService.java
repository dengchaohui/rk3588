...
public final class CarOccupantZoneService extends ICarOccupantZone.Stub
    ...
    final DisplayManager.DisplayListener mDisplayListener =
            new DisplayManager.DisplayListener() {
                ...
                public void onDisplayAdded(int displayId) {
                    Slogf.w(TAG, "add displayId = " + displayId);
                    handleDisplayChange();
                }

                @Override
                public void onDisplayRemoved(int displayId) {
                    Slogf.w(TAG, "remove displayId = " + displayId);
                    handleDisplayChange();
                }
    ...
    public void init() {
        ...
        CarUserService.ZoneUserBindingHelper helper = new CarUserService.ZoneUserBindingHelper() {
            ...
            public boolean assignUserToOccupantZone(@UserIdInt int userId, int zoneId) {
                Slogf.i(TAG, "> assignUserToOccupantZone userid=%d zoneid=%d", userId, zoneId);
                ...
                synchronized (mLock) {
                    ...
                    if (zoneConfig.userId != UserHandle.USER_NULL && zoneConfig.userId != 0 && zoneConfig.userId != userId) {
                        Slogf.w(TAG, "other user already occupies the zone(%d) zoneuid(%d) != uid(%d)", zoneId, zoneConfig.userId, userId);
                        return false;
                    }
    ...
    public boolean assignProfileUserToOccupantZone(int occupantZoneId, int userId) {
        Slogf.i(TAG, "assignProfileUserToOccupantZone occupantZoneId=%d, userId=%d", occupantZoneId, userId);
    ...
    private void handleActiveDisplaysLocked() {
        ...
        for (int i = 0; i < mActiveOccupantConfigs.size(); ++i) {
            ...
            if (occupantConfig != null) {
                Slogf.w(TAG, "occupantConfig.display.clear(), " + occupantConfig.toString());
    ...
    private void handleUserChangesLocked() {
        ...
        Slogf.w(TAG, "handleUserChangesLocked, currentUid(%d)", driverUserId);  // TODO: bugs here, id is 0
