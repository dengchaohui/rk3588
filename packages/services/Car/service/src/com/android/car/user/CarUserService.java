...
public final class CarUserService extends ICarUserService.Stub implements CarServiceBase {
    ...
    private void onUserSwitching(@UserIdInt int fromUserId, @UserIdInt int toUserId) {
        ...
        if (mEnablePassengerSupport && isPassengerDisplayAvailable()) {
            ...
            int count = setupAndRunPassengers(toUserId);
            Slog.d(TAG, "Total passenger: " + count);
        }
        t.traceEnd();
    }
    ...
    /**
     * Setup and starts all passengers for occupant zones
     * 
     * @param driverId
     * @return passengers size
    */
    private int setupAndRunPassengers(int driverId) {
        int currentUser = ActivityManager.getCurrentUser();
        int passengerIndex = 0;
        int[] occupantTypes = new int[] {
        ...
        for (int occupantType : occupantTypes) {
            ...
            synchronized (mLockHelper) {
                if (mZoneUserBindingHelper == null) {
                    Slog.e(TAG, "implementation is not delegated");
                    return 0;
                }
                helper = mZoneUserBindingHelper;
            }
            List<OccupantZoneInfo> zoneInfos = helper.getOccupantZones(occupantType);
            for (OccupantZoneInfo zoneInfo : zoneInfos) {  // Each zone
                ...
                if (profileCount <= passengerIndex) {
                    UserInfo passenger = createPassenger("Passenger" + passengerIndex, currentUser);
                    if (passenger == null) {
                        // Couldn't create user, most likely because there are too many.
                        Slog.w(TAG, "cannot create a passenger user" + passengerIndex);
                        return passengerIndex;
                    }
                }
                // Start passenger
                List<UserInfo> passengers = getPassengers(driverId);
                if (passengers.size() > passengerIndex) {
                    int passengerId = passengers.get(passengerIndex).id;
