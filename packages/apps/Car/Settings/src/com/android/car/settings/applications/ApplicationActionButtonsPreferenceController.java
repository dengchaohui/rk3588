...
public class ApplicationActionButtonsPreferenceController extends
    ...
    private final View.OnClickListener mUninstallClickListener = i -> {
        ...
        if (mAppEntry != null) {
            android.util.Log.d("ApplicationActionButtonsPreferenceController", "uninstallDeviceAdmin AppEntry = " + mAppEntry.info.packageName + ", uid = " + mAppEntry.info.uid);
        }
        ...
        if (mDpm.packageHasActiveAdmins(mPackageName)) {
            ...
            if (mAppEntry != null) {
                UserHandle mUserHandle = UserHandle.getUserHandleForUid(mAppEntry.info.uid);
                UserInfo mUserInfo = mUserManager.getUserInfo(mUserHandle.getIdentifier());
                deviceAdminIntent.putExtra(Intent.EXTRA_USER, mUserHandle);
                if (mUserHandle.isSystem() || mUserInfo.isAdmin()) {
                    deviceAdminIntent.putExtra(Intent.EXTRA_UNINSTALL_ALL_USERS, true);
                }
            }
            ...
            if (mAppEntry != null) {
                UserHandle mUserHandle = UserHandle.getUserHandleForUid(mAppEntry.info.uid);
                UserInfo mUserInfo = mUserManager.getUserInfo(mUserHandle.getIdentifier());
                uninstallIntent.putExtra(Intent.EXTRA_USER, mUserHandle);
                if (mUserHandle.isSystem() || mUserInfo.isAdmin()) {
