...
//    int mDeviceFd = open(deviceName, O_RDWR | O_NONBLOCK, 0);
    ...
    LOG(DEBUG) << "Open Device: " << deviceName << " (fd = " << mDeviceFd << ")";
...
bool VideoCapture::startStream(std::function<void(VideoCapture*, imageBuffer*, void*)> callback) {
    ...
    mBufferPlanes = std::make_unique<v4l2_plane[]>(mNumBuffers);
    ...
    for (int i = 0; i < mNumBuffers; ++i) {
      // Get the information on the buffer that was created for us
        ...
        memset(&mBufferPlanes[i], 0, sizeof(v4l2_plane));
        mBufferInfos[i].type = mBuftype;
        mBufferInfos[i].memory = V4L2_MEMORY_MMAP;
        mBufferInfos[i].index = i;
        if (V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE == mBuftype) {
            mBufferInfos[i].m.planes = &mBufferPlanes[i];
            mBufferInfos[i].length = mFormatPlanes;
        }
        ...
        } else if (V4L2_BUF_TYPE_VIDEO_CAPTURE == mBuftype) {
            LOG(INFO) << "Buffer description:";
            LOG(INFO) << "  buffer length: " << mBufferInfos[i].length;
            LOG(INFO) << "  offset: " << mBufferInfos[i].m.offset;
            LOG(INFO) << "  flags : " << std::hex << mBufferInfos[i].flags;

            // Get a pointer to the buffer contents by mapping into our address space
            mBufferLength = mBufferInfos[i].length;
            mPixelBuffers[i] = mmap(
                NULL,
        ...
        memset(mPixelBuffers[i], 0, mBufferLength);
...
void VideoCapture::stopStream() {
    ...
    for (int i = 0; i < mNumBuffers; ++i) {
        ...
        munmap(mPixelBuffers[i], mBufferLength);
    }
...
void VideoCapture::collectFrames() {
    ...
    while (mRunMode == RUN) {
        struct v4l2_plane planes[mFormatPlanes];
        ...
        if (V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE == mBuftype) {
            memcpy(&mBufferPlanes[buf.index], buf.m.planes, sizeof(v4l2_plane) * buf.length);
            mBufferInfos[buf.index] = buf;
            mBufferInfos[buf.index].m.planes = &mBufferPlanes[buf.index];
        } else if (V4L2_BUF_TYPE_VIDEO_CAPTURE == mBuftype)
            mBufferInfos[buf.index] = buf;
