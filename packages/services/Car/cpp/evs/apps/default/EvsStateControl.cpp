...
bool EvsStateControl::configureEvsPipeline(State desiredState) {
    ...
    if (!isGlReady && !isSfReady()) {
        ...
        if (mCameraList[desiredState].size() >= 1) {
            if (mCurRenderType != RenderType::COPY) {
                mDesiredRenderer =
                        std::make_unique<RenderPixelCopy>(mEvs, mCameraList[desiredState][0]);
                if (!mDesiredRenderer) {
                    LOG(ERROR)
                            << "Failed to construct Pixel Copy renderer.  Skipping state change.";
                    return false;
                }
            }
            type = RenderType::COPY;
        } else {
        ...
        if (mCameraList[desiredState].size() == 1) {
            if (mCurRenderType != RenderType::DIRECT) {
    ...
    if (mCurRenderType == RenderType::NONE && mCurrentRenderer != nullptr) {
        mCurrentRenderer->deactivate();
