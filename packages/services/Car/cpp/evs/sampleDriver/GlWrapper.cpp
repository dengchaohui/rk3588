...
bool GlWrapper::updateImageTexture(const BufferDesc_1_1& aFrame) {
    ...
    if (mKHRimage == EGL_NO_IMAGE_KHR) {
		...
		int ret = AHardwareBuffer_createFromHandle(pDesc,
                                                     const_cast<native_handle_t*>(aFrame.buffer.nativeHandle.getNativeHandle()),
                                                     AHARDWAREBUFFER_CREATE_FROM_HANDLE_METHOD_CLONE, 
                                                     &mHardwareBuffer
		);
		if (ret) {
            LOG(ERROR) << "AHardwareBuffer_createFromHandle failed. ret:" << ret;
            abort();
        }

        EGLint eglImageAttributes[] = {EGL_IMAGE_PRESERVED_KHR, EGL_TRUE, EGL_NONE};
        EGLClientBuffer cbuf = eglGetNativeClientBufferANDROID(mHardwareBuffer);
