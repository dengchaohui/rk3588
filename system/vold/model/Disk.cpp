...
void Disk::createPublicVolume(dev_t device) {
    ...
    if (!strcmp(product, "car") || !strcmp(product, "vehicle")) {
       ...
       for (userid_t user : vol_manager->getStartedUsers()) {
            if(user == 0 )//ignore user 0
            {
                LOG(WARNING) << "createPublicVolume ignore user0 ";
                continue;
            }
            ...
            vol->setId(StringPrintf("%s%d",vol->getId().c_str(),user));
            LOG(WARNING) << "createPublicVolume reset id to " << vol->getId();
