		...
		bool is16Align, bool isViraddr)
{
    ...
    dst_width  = (dst_width  + 15) & (~15);
    dst_height = (dst_height + 15) & (~15);
    ...
    rga_set_rect(&src.rect, zoom_left_offset, zoom_top_offset,
                ...
                src_height, src_fmt/*HAL_PIXEL_FORMAT_RGBA_8888*/);

        rga_set_rect(&dst.rect, 0,0,dst_width,dst_height,
                    dst_width,dst_height,
                    dst_fmt/*HAL_PIXEL_FORMAT_RGBA_8888*/);
		...
		int src_fmt, int dst_fmt, bool isFd)
{
	...
	LOGD("%s:%d: rga simple src fd %d dst fd %d \n", __FUNCTION__, __LINE__, src.fd, dst.fd);
