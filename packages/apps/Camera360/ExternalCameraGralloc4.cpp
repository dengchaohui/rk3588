...
status_t validateBufferDescriptorInfo(
    ...
    if (descriptorInfo->width == 0 || descriptorInfo->height == 0 || descriptorInfo->layerCount == 0)
        {
                LOGE("size or layoutcount error");
                return android::BAD_VALUE;
        }
...
status_t createDescriptor(void* bufferDescriptorInfo,
    ...
    LOGD("createDescriptor %d %d %d",descriptorInfo->width, descriptorInfo->height ,descriptorInfo->layerCount);
...
static int allocate_gralloc_buffer(size_t width,
    ...
    LOGD("allocate_gralloc_buffer %d %d %d",descriptorInfo.width, descriptorInfo.height ,descriptorInfo.layerCount);
...
static cam_mem_info_t* cam_mem_gralloc_ops_alloc(
        cam_mem_handle_t* handle, size_t size, unsigned int halPixFmt,
