...
public class CarSystemBar extends SystemUI implements CommandQueue.Callbacks {
    ...
    private List<ViewGroup> mSecondaryBottomSystemBarWindows;
    ...
    private List<CarSystemBarView> mSecondaryBottomSystemBarViews;
    ...
    private List<Context> mDisplayContext;
    ...
    public CarSystemBar(Context context,
        ...
        mDisplayContext = new ArrayList<>();
        mSecondaryBottomSystemBarWindows = new ArrayList<>();
        mSecondaryBottomSystemBarViews = new ArrayList<>();
    ...
    private void restartNavBars() {
        ...
        for (int i = 0; i < mSecondaryBottomSystemBarWindows.size(); ++i) {
            mSecondaryBottomSystemBarWindows.get(i).removeAllViews();
            mHvacController.unregisterViews(mSecondaryBottomSystemBarViews.get(i));
        }
        mSecondaryBottomSystemBarViews.clear();
    ...
    private void buildNavBarWindows() {
        ...
        for (int i = 0; i < mDisplayContext.size(); ++i) {
            mSecondaryBottomSystemBarWindows.add(mCarSystemBarController.getSecondaryBottomWindow(mDisplayContext.get(i)));
        }
    }
    ...
    private void buildNavBarContent() {
        ...
        for (int i = 0; i < mDisplayContext.size(); ++i) {
            CarSystemBarView secondaryBottomSystemBarView = mCarSystemBarController.getSecondaryBottomBar(mDisplayContext.get(i), isDeviceSetupForUser());
            Log.d(TAG, "insetSystemBar:" + i + " ; dcontext=" + mDisplayContext.get(i) + " ; barview=" + secondaryBottomSystemBarView);
            if (secondaryBottomSystemBarView != null) {
                mSystemBarConfigs.insetSystemBar(SystemBarConfigs.BOTTOM2, secondaryBottomSystemBarView);
                mHvacController.registerHvacViews(secondaryBottomSystemBarView);
                mSecondaryBottomSystemBarWindows.get(i).addView(secondaryBottomSystemBarView);
                mSecondaryBottomSystemBarViews.add(secondaryBottomSystemBarView);
            }
        }
        assert(mDisplayContext.size() == mSecondaryBottomSystemBarWindows.size());
        assert(mDisplayContext.size() == mSecondaryBottomSystemBarViews.size());
    }
    ...
    private void attachNavBarBySide(int side) {
        switch (side) {
            ...
            case SystemBarConfigs.RIGHT:
                ...
                break;
            case SystemBarConfigs.BOTTOM2:
                for (int i = 0; i < mDisplayContext.size(); ++i) {
                    if (mSecondaryBottomSystemBarWindows.get(i) != null) {
                        WindowManager wm = (WindowManager)mDisplayContext.get(i).getSystemService(Context.WINDOW_SERVICE);
                        if (wm.shouldShowSystemDecors(mDisplayId)) {
                            Log.w(TAG, "Secondary BOTTOM For secondary display:" + i + " ; " + wm);
                            wm.addView(mSecondaryBottomSystemBarWindows.get(i),
                                mSystemBarConfigs.getLayoutParamsBySide(SystemBarConfigs.BOTTOM2));
                        } else {
                            Log.w(TAG, "Secondary BOTTOM For secondary non-display:" + i);
                        }
                    }
                }
                break;
    ...
    public void setImeWindowStatus(int displayId, IBinder token, int vis, int backDisposition,
        ...
        if (mContext.getDisplayId() != displayId) {
            if (mHideBottomBarForKeyboard) {
                for (Context displayContext : mDisplayContext) {
                    if (displayContext.getDisplayId() == displayId) {
                        Log.d(TAG, "displayId = " + displayContext.getDisplayId() + " setSecondaryBottomWindowVisibility");
                        mCarSystemBarController.setSecondaryBottomWindowVisibility(
                                displayContext, isKeyboardVisible ? View.GONE : View.VISIBLE);
                        break;
                    }
                }
            }
    ...
    private void initCarApi() {
        ...
        mCarApi = Car.createCar(mContext, null, Car.CAR_WAIT_TIMEOUT_WAIT_FOREVER,
                (Car car, boolean ready) -> {
                    ...
                    for(OccupantZoneInfo zone : zones) {
                        if (zone.occupantType == CarOccupantZoneManager.OCCUPANT_TYPE_DRIVER) {
                            clusterDisplay = mOccupantZoneManager.getDisplayForOccupant(
                                zone, CarOccupantZoneManager.DISPLAY_TYPE_INSTRUMENT_CLUSTER);
                            break;
                        }
                    }

                    Log.w(TAG, ">> new CarSystemBar: displayid=" + mDisplayId);
                    DisplayManager dm = (DisplayManager) mContext.getSystemService(Context.DISPLAY_SERVICE);
                    Display dispArray[] = dm.getDisplays();

                    for (int i = 1; i < dispArray.length; ++i) {
                        if (clusterDisplay != null && dispArray[i].getDisplayId() == clusterDisplay.getDisplayId()) {
                            Log.i(TAG, "ignore display id = " + dispArray[i].getDisplayId() + ", clusterDisplay = " + clusterDisplay.toString());
                            continue;
                        }
                        mDisplayContext.add(mContext.createDisplayContext(dispArray[i]));
                    }
                });
    }
}
