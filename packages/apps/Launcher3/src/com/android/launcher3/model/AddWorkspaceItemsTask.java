...
public class AddWorkspaceItemsTask extends BaseModelUpdateTask {
    ...
    public void execute(LauncherAppState app, BgDataModel dataModel, AllAppsList apps) {
		...
		// 尝试先放在第一页 add start try
                int[] coords = findSpaceOnSpecificScreen(app, dataModel,
                        workspaceScreens.get(0), item.spanX, item.spanY);

                int screenId;
                if (coords != null) {
                    screenId = workspaceScreens.get(0);
                } else {

                // Find appropriate space for the item.
                	coords = findSpaceForItem(app, dataModel, workspaceScreens,
     ...
     * 在指定的 screenId 上查找空位
     */
    private int[] findSpaceOnSpecificScreen(LauncherAppState app, BgDataModel dataModel,
                                            int screenId, int spanX, int spanY) {
        InvariantDeviceProfile idp = app.getInvariantDeviceProfile();
        int cellXCount = idp.numColumns;
        int cellYCount = idp.numRows;

        boolean[][] occupied = new boolean[cellXCount][cellYCount];
        for (ItemInfo info : dataModel.workspaceItems) {
            if (info.screenId == screenId) {
                for (int x = info.cellX; x < info.cellX + info.spanX; x++) {
                    for (int y = info.cellY; y < info.cellY + info.spanY; y++) {
                        if (x >= 0 && x < cellXCount && y >= 0 && y < cellYCount) {
                            occupied[x][y] = true;
                        }
                    }
                }
