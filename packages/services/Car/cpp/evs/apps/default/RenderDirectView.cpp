...
bool RenderDirectView::activate() {
    ...
    LOG(INFO) << "RenderDirectView::activate";
...
//       return false;
    ...
    LOG(INFO) << "RenderDirectView::activate done";
    return true;
}
...
void RenderDirectView::deactivate() {
    std::lock_guard<std::mutex> lg(sMutex);
    LOG(INFO) << "RenderDirectView::deactivate";
    ...
    // the time.
    mTexture = nullptr;
    LOG(INFO) << "RenderDirectView::deactivate done";
}
