...
sp<DisplayDevice> SurfaceFlinger::setupNewDisplayDeviceInternal(
    ...
    if(internalDisplayOrientation == ui::ROTATION_90 ||
       internalDisplayOrientation == ui::ROTATION_270){
        ...
        display->setProjection(state.orientation, Rect(display->getHeight(), display->getWidth()),
                                                  Rect(display->getHeight(), display->getWidth()));
    }
    else{
        /*ALOGE("rk-debug[%s %d] name:%s displayInstallOrientation:%d \n",
                __FUNCTION__,__LINE__,state.displayName.c_str(),displayInstallOrientation);*/
